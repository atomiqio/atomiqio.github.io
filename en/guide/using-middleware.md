...
_Middleware_ functions are functions that have access to the [request object](/{{ page.lang }}/1x/api.html#req)  (`req`), the [response object](/{{ page.lang }}/1x/api.html#res) (`res`), and the next middleware function in the application's request-response cycle. The next middleware function is commonly denoted by a variable named `next`.
...
Bind application-level middleware to an instance of the [app object](/{{ page.lang }}/1x/api.html#app) by using the `app.use()` and `app.METHOD()` functions, where `METHOD` is the HTTP method of the request that the middleware function handles (such as GET, PUT, or POST) in lowercase.
...
For information on the `options` argument and more details on this middleware function, see [express.static](/en/1x/api.html#express.static).
